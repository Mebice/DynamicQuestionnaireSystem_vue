    ...
    saveWithoutPublish() {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          const requestBody = {
            questionnaire: {
              title: this.previewData.questionnaire,
...
<template>
  <div class="bgArea">
    ...
    <div class="titleArea">
      ...
      <h2>{{ previewData.questionnaire }}</h2>
        ...
        <div class="questionListInside">
          ...
          <div v-else>
            <div class="option-radio" v-for="(option, optionIndex) in item.options" :key="optionIndex">
              <input type="radio" :id="`option_${index}_${optionIndex}`" :value="option" v-model="item.selectedOption"
                :disabled="true" />
              <label class="optionText" :for="`option_${index}_${optionIndex}`">{{ option }}</label>
            </div>
          </div>
    ...
    <div class="btnArea">
      ...
      <div class="buttons">
        ...
        <!-- <button class="btn" @click="saveWithoutPublish">保存不發布</button> -->

        <!-- <button class="btn" @click="savePublish">保存並發佈</button> -->
